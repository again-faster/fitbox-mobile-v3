...
describe('Weekly Recap screen', () => {
	...
	it('requires an authenticated member session and enabled recap feature', () => {
		...
		expect(
			hasMemberRecapSession({
				user: { id: 'solo-1', persona: 'solo', active_tenant_id: 'tenant-1' },
			} as never),
		).toBe(false);
		expect(hasMemberRecapSession(memberSession as never)).toBe(true);
		expect(shouldEnableWeeklyRecapQuery(false, memberSession as never)).toBe(
			false,
		);
	...
	it('maps query conditions to loading, error, empty, and ready states', () => {
		expect(weeklyRecapViewState(false, false, false, undefined)).toBe('empty');
		expect(weeklyRecapViewState(true, true, false, undefined)).toBe('loading');
		expect(weeklyRecapViewState(true, false, true, undefined)).toBe('error');
		expect(weeklyRecapViewState(true, false, false, null)).toBe('empty');
		expect(weeklyRecapViewState(true, false, false, snapshot)).toBe('ready');
	});
