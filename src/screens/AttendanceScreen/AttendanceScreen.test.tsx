...
describe('AttendanceScreen graph loading', () => {
	it('renders a visible retry action for an HTTP-200 month graph error', async () => {
		...
		await waitFor(() =>
			expect(
				screen.getByText("We couldn't load your monthly attendance chart."),
			).toBeTruthy(),
		);
	...
	it('ignores an older month response after changing the year filter', async () => {
		...
		await waitFor(() =>
			expect(screen.getByTestId('attendance-chart-values')).toHaveTextContent(
				'7',
		),
		);
		expect(screen.getByTestId('attendance-chart-values')).not.toHaveTextContent(
			'99',
		);
	});
