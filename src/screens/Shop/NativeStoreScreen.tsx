...
const StoreScreen = ({
	...
	<SafeAreaView style={[styles.screen, style]}>
		<View style={[styles.screenContainer, contentContainerStyle]}>
			{children}
		</View>
	</SafeAreaView>
);
...
const StoreButton = ({
	...
	<Pressable
		...
		style={[
			styles.storeButton,
			compact ? styles.compactButton : styles.primaryButton,
...
> = {
	default: {
		background: memberTheme.colors.surfaceSoft,
		foreground: memberTheme.colors.primaryInk,
	},
...
const StoreStatusPill = ({
	...
	<View
		style={[
			styles.statusPill,
			{ backgroundColor: statusPalette[status].background },
		]}
		accessible
...
export default function NativeStoreScreen({
	...
	const [selectedOrder, setSelectedOrder] =
		useState<NativeOrderDetail | null>(null);
	const [selectedOrderLoading, setSelectedOrderLoading] = useState(false);
	const [checkoutOpen, setCheckoutOpen] = useState(false);
	const [selectedProduct, setSelectedProduct] =
		useState<NativeStoreProduct | null>(null);
	const [confirmation, setConfirmation] =
		useState<NativeCheckoutResponse | null>(null);
			...
			try {
				...
				if (!isNativeStoreResponse(nextStore)) {
					throw new NativeCommerceError(
						'legacy_fallback',
						'This gym is still using its existing store.',
						404,
					);
				}
				setStore(nextStore);
	...
	const addToCart = async (
		...
		variantId = primaryVariant(product)?.variant_id,
	) => {
		const variant =
			product.variants.find(item => item.variant_id === variantId) ??
	...
	const updateCartQuantity = async (variantId: string, quantity: number) => {
		...
		try {
			...
			setCart(
				await nativeCommerce.replaceCart(
					identity,
					setCartLineQuantity(currentLines, variantId, quantity),
				),
			);
		} catch (cause) {
			setError(
	...
	const removeFromCart = async (variantId: string) => {
		...
		try {
			...
			setCart(
				await nativeCommerce.replaceCart(
					identity,
					removeCartLine(currentLines, variantId),
				),
			);
		} catch (cause) {
			setError(
	...
	const selectOrder = async (order: NativeOrderSummary) => {
		...
		setOrdersError(null);
		try {
			const response = await nativeCommerce.getOrder(
				identity,
				order.order_id,
			);
	...
	const checkout = async () => {
		...
		try {
			...
			const prepared = await nativeCommerce.prepareCheckout(
				identity,
				checkoutAddress,
			);
			...
			const paymentSheet = await initPaymentSheet({
				...
				paymentIntentClientSecret:
					prepared.payment_intent_client_secret,
		...
		return (
			<MemberScreen>
				<View style={styles.loading}>
					...
					/>
					<MemberText muted>Loading your gym store…</MemberText>
				</View>
			</MemberScreen>
		);
	}
	...
	return (
		<MemberScreen contentContainerStyle={styles.screenContent}>
			<FlatList
				...
				refreshControl={
					<RefreshControl
						...
						tintColor={memberTheme.colors.primary}
					/>
				}
				ListHeaderComponent={
					<>
						<View style={styles.headerRow}>
							<View>
								...
								<MemberText muted>
									Essentials for your training week.
								</MemberText>
								{store?.store_mode === 'shadow' ? (
									<MemberText
										muted
										style={styles.shadowNotice}
									>
						...
						/>
						{error ? (
							<MemberText muted style={styles.inlineError}>
								{error}
							</MemberText>
						) : null}
					</>
				...
				renderItem={({ item }) => (
					<ProductCard
						product={item}
						busy={
							busyVariantId === primaryVariant(item)?.variant_id
						}
			...
			/>
			{notice ? (
				<NoticePanel message={notice} onClose={() => setNotice(null)} />
			) : null}
			{confirmation ? (
				<ConfirmationPanel
			...
			{selectedProduct ? (
				<ProductDetailPanel
					...
					busy={busyVariantId !== null}
					onClose={() => setSelectedProduct(null)}
					onAdd={variantId => {
						void addToCart(selectedProduct, variantId);
...
function statusKind(
	...
	if (
		['needs_review', 'payment_failed', 'cancelled', 'refunded'].includes(
			status,
		)
	)
		return 'danger';
	if (
		['shipped', 'partially_shipped', 'partially_fulfilled'].includes(status)
	)
		return 'info';
	if (
		['pending_payment', 'unfulfilled', 'submitted', 'confirmed'].includes(
			status,
		)
	)
		return 'warning';
...
function OrdersPanel({
	...
	return (
		<View style={styles.ordersPanel}>
			...
			>
				{error ? (
					<MemberText muted style={styles.checkoutError}>
						{error}
					</MemberText>
				) : null}
				{selectedOrder ? (
	...
	return (
		<View>
			<View style={styles.orderSummary}>
				<View style={styles.orderPills}>
					...
					<MemberStatusPill
						...
						status={statusKind(order.payment_status)}
					/>
				</View>
				<MemberText muted>
			...
			{order.lines.map(line => (
				<View key={line.line_id} style={styles.detailLine}>
					<View style={styles.orderRowCopy}>
						<MemberText>
							{line.quantity} × {line.title}
						</MemberText>
						{line.variant_label ? (
							<MemberText muted>{line.variant_label}</MemberText>
						) : null}
					</View>
			...
			<View style={styles.orderTotals}>
				<View style={styles.detailLine}>
					...
					<MemberText muted>{money(order.subtotal)}</MemberText>
				</View>
				<View style={styles.detailLine}>
					<MemberText muted>Shipping</MemberText>
...
function ConfirmationPanel({
	...
	return (
		<View style={styles.confirmationPanel} accessibilityRole="alert">
			...
			<MemberText muted>
				{simulated
					? 'No payment was taken and no supplier was notified.'
					: 'Your payment is being confirmed securely. We’ll update you when it ships.'}
			</MemberText>
			<View style={styles.orderTotals}>
				<View style={styles.detailLine}>
					<MemberText muted>Subtotal</MemberText>
					<MemberText muted>
						{money(confirmation.subtotal)}
					</MemberText>
				</View>
				<View style={styles.detailLine}>
					<MemberText muted>Shipping</MemberText>
					<MemberText muted>
						{money(confirmation.shipping)}
					</MemberText>
				</View>
				<View style={styles.detailLine}>
					<MemberText role="label">Total</MemberText>
...
function ProductDetailPanel({
	...
	const selectedVariant =
		product.variants.find(variant => variant.variant_id === variantId) ??
		firstVariant;
	const unavailable =
		!selectedVariant || selectedVariant.stock_status === 'out_of_stock';
	...
	return (
		<View style={styles.productDetailPanel}>
			...
			>
				...
				{product.description ? (
					<MemberText muted style={styles.detailDescription}>
						{product.description}
					</MemberText>
				) : null}
				<MemberText role="label" style={styles.detailHeading}>
				...
				<View style={styles.variantList}>
					{product.variants.map(variant => (
						<MemberPill
							...
							selected={variant.variant_id === variantId}
							onPress={() => setVariantId(variant.variant_id)}
						/>
					))}
				</View>
...
function ProductCard({
	...
	return (
		<MemberCard elevated={false} style={styles.productCard}>
			<Pressable
				onPress={onOpen}
				accessibilityRole="button"
			...
			>
				{product.image_url ? (
					...
					<View style={styles.productImagePlaceholder}>
						<MemberText muted>No image</MemberText>
					</View>
				)}
				<MemberText
					role="label"
	...
	return (
		<View style={styles.cartPanel}>
			...
			{cart?.lines.length ? (
				<>
					{cart.lines.map(line => (
						<View key={line.line_id} style={styles.cartLine}>
							<View style={styles.cartLineCopy}>
								<MemberText>{line.title}</MemberText>
								{line.variant_label ? (
									<MemberText muted>
										{line.variant_label}
									</MemberText>
							...
							<View style={styles.cartLineActions}>
								...
								<Pressable
									disabled={busyVariantId === line.variant_id}
									onPress={() => onRemove(line.variant_id)}
									accessibilityRole="button"
							...
							{cart.shipping_groups.map(group => (
								...
								>
									<View>
										...
										<MemberText muted>
											{money(group.merchandise_subtotal)}
										</MemberText>
									</View>
									<MemberText muted>
										{group.free_shipping
											? 'Free'
											: money(group.shipping_amount)}
									</MemberText>
								</View>
							))}
						</View>
					) : (
						<MemberText muted style={styles.cartNote}>
							Shipping is calculated using the global supplier and
							country rules. You’ll confirm the total before
							payment.
						</MemberText>
					)}
				...
				<MemberText muted>Your cart is empty.</MemberText>
			)}
		</View>
	);
}
	...
	return (
		<View style={styles.checkoutPanel}>
			...
			>
				...
				/>
				<MemberText muted style={styles.cartNote}>
					Payment is processed securely by Stripe. Shipping rates are
					fixed for now and there are no live carrier quotes.
				</MemberText>
			</ScrollView>
		</View>
	);
...
const styles = StyleSheet.create({
	...
	storeButton: {
		minHeight: storeLayout.minTouchTarget,
	...
	primaryButton: {
		minHeight: storeLayout.primaryHeight,
	...
	headerRow: {
		paddingHorizontal: storeLayout.screenGutter,
		flexDirection: 'row',
	...
	search: {
		marginHorizontal: storeLayout.screenGutter,
	...
	quantityButton: {
		...
		borderRadius: memberTheme.radius.pill,
		borderWidth: 1,
		height: storeLayout.minTouchTarget,
		justifyContent: 'center',
		width: storeLayout.minTouchTarget,
	},
	...
	noticePanel: {
		...
		right: memberTheme.spacing.lg,
		top: memberTheme.spacing.lg,
		zIndex: 10,
		backgroundColor: memberTheme.colors.surfaceSoft,
		borderColor: memberTheme.colors.border,
		borderRadius: memberTheme.radius.md,
	...
	ordersLoading: {
		alignItems: 'center',
		gap: memberTheme.spacing.md,
		paddingVertical: memberTheme.spacing.xl,
	},
	ordersEmpty: {
		alignItems: 'center',
		paddingVertical: memberTheme.spacing.xxl,
	},
	orderRow: {
		flexDirection: 'row',
		alignItems: 'center',
