...
const isMoney = (value: unknown): boolean =>
  isRecord(value) && typeof value.currency === 'string' && typeof value.minor === 'number';
 ...
 */
export const parseNativeCommerceResponse = <T>(action: string, payload: unknown): T => {
  ...
  const valid = (() => {
    ...
    switch (action) {
      case 'store':
        if ((record.store_mode === 'legacy' || record.store_mode === 'paused') && typeof record.fallback_url === 'string') return true;
        return (record.store_mode === 'shadow' || record.store_mode === 'native') &&
          typeof record.gym_id === 'number' &&
          Array.isArray(record.products) &&
          Array.isArray(record.categories);
      case 'cart.get':
      case 'cart.replace':
        return typeof record.gym_id === 'number' &&
          typeof record.version === 'number' &&
          Array.isArray(record.lines) &&
          isMoney(record.subtotal) &&
          (!('shipping_groups' in record) || (Array.isArray(record.shipping_groups) && record.shipping_groups.every(isShippingGroup)));
      case 'checkout.prepare':
        return typeof record.gym_id === 'number' &&
          typeof record.order_id === 'string' &&
          (typeof record.order_number === 'number' || typeof record.order_number === 'string') &&
          isMoney(record.subtotal) &&
          ...
          (!('shipping_groups' in record) || (Array.isArray(record.shipping_groups) && record.shipping_groups.every(isShippingGroup)));
      case 'orders.list':
        return typeof record.gym_id === 'number' && Array.isArray(record.orders);
      case 'orders.get':
        return typeof record.gym_id === 'number' && isRecord(record.order);
      default:
        return true;
    }
  })();

  if (!valid) throw new Error(`Invalid native commerce response for ${action}`);
  return payload as T;
};
