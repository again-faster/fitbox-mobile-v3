...
const request = async <T>(
  ...
  const response = await fetch(nativeCommerceEndpoint(Constant.COMMERCE_SUPABASE_URL), {
    method: 'POST',
    headers: buildNativeCommerceHeaders(identity, Constant.COMMERCE_SUPABASE_ANON_KEY),
  ...
  return parseNativeCommerceResponse<T>(String(body.action ?? 'unknown'), payload);
};
...
export const nativeCommerce = {
  getStore: (identity: NativeCommerceIdentity, query?: string, categorySlug?: string) => request<NativeStoreResponse | NativeStoreFallbackResponse>(identity, {
    action: 'store',
    ...(query ? { query } : {}),
    ...(categorySlug ? { category_slug: categorySlug } : {}),
  }),
  getCart: (identity: NativeCommerceIdentity) => request<NativeCartResponse>(identity, { action: 'cart.get' }),
  replaceCart: (identity: NativeCommerceIdentity, lines: Array<{ variant_id: string; quantity: number }>) => request<NativeCartResponse>(identity, {
    action: 'cart.replace',
    request_key: requestKey('cart'),
    lines,
  }),
  prepareCheckout: (identity: NativeCommerceIdentity, shippingAddress: Record<string, string>) => request<NativeCheckoutResponse>(identity, {
    action: 'checkout.prepare',
    idempotency_key: requestKey('checkout'),
    shipping_address: shippingAddress,
  }),
  getOrders: (identity: NativeCommerceIdentity, cursor?: string) => request<{ request_id: string; gym_id: number; orders: NativeOrderSummary[] }>(identity, {
    action: 'orders.list',
    ...(cursor ? { cursor } : {}),
  }),
  getOrder: (identity: NativeCommerceIdentity, orderId: string) => request<{ request_id: string; gym_id: number; order: NativeOrderDetail }>(identity, {
    action: 'orders.get',
    order_id: orderId,
  }),
};
