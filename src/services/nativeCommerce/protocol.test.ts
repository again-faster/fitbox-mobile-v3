...
describe('native commerce mobile protocol', () => {
  it('builds headers from the existing Fitbox signed store session', () => {
    expect(buildNativeCommerceHeaders({
  ...
  it('rejects malformed member responses before the UI consumes them', () => {
    expect(() => parseNativeCommerceResponse('store', { request_id: 'r1' })).toThrow(
      'Invalid native commerce response',
    );
  });
  ...
  it('accepts the canonical shadow store and supplier shipping groups', () => {
    ...
    expect(parseNativeCommerceResponse('store', {
    ...
    expect(parseNativeCommerceResponse('cart.get', {
      request_id: 'r2', gym_id: 231, version: 1, lines: [], subtotal: { currency: 'AUD', minor: 12000 }, shipping_groups: [shippingGroup],
  ...
  it('accepts explicit legacy fallback responses and rejects malformed shipping groups', () => {
    expect(parseNativeCommerceResponse('store', {
      request_id: 'r1', gym_id: 231, store_mode: 'legacy', fallback_url: 'https://store.fitbox.iq/evolutionfit',
